...
class AuthProvider {
  ...
  auth(sendAuthCommand, credentials, callback) {
    ...
    const execute = () => {
      this._authenticateSingleConnection(sendAuthCommand, credentials, (err, r) => {
        // Adjust count
        count = count - 1

        // If we have an error
        if (err) {
          errorObject = createError(err)
        } else if (r && (r.$err || r.errmsg)) {
          errorObject = createError(r)
        } else {
        ...
        if (numberOfValidConnections > 0) {
          ...
          if (errorObject == null) {
            errorObject = createError(`failed to authenticate using ${credentials.mechanism}`)
          }
   ...
   */
  reauthenticate(sendAuthCommand, callback) {
    ...
    for (let i = 0; i < authStore.length; i++) {
      this.auth(sendAuthCommand, authStore[i], (err) => {
        count = count - 1
        if (count === 0) {
          callback(err, null)
        }
